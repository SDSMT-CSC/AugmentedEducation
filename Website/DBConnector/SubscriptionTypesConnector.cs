...
{
    public class SubscriptionTypesConnector : DBConnector
    {
        #region Members
        ...
        public ResultPackage<bool> Insert(long permission_level_id, string permission_level_name)
        {
            SqlCommand command = new SqlCommand(
                $"INSERT INTO {_Table_Name} (SubscriptionTypeId, SubscriptionTypeName) " +
                "VALUES(@SubscriptionTypeId, @SubscriptionTypeName)");

            Add_Command_Parameter_With_Value(FieldNames.SubscriptionTypeId, permission_level_id, ref command);
            Add_Command_Parameter_With_Value(FieldNames.SubscriptionTypeName, permission_level_name, ref command);
            ...
            if (value != null)
            {
                switch (field_name)
                {
                    case FieldNames.SubscriptionTypeId:
                        ...
                        break;
                    case FieldNames.SubscriptionTypeName:
                        valid_value = Verify_PermissionName(value);
                        if (string.IsNullOrEmpty(valid_value.ErrorMessage))
                        {
                            if (string.IsNullOrEmpty(preferred_name))
                                preferred_name = "@SubscriptionTypeName";
        ...
        private ResultPackage<bool> Verify_PerimissionId(object permission_id)
        {
                ...
                permission_id.GetType() == typeof(int))
            {
                ...
                if (permission_id_long < 0)
                    permission_id_result.ErrorMessage = "SubscriptionTypeId must be greater than 0";
            }
            else { permission_id_result.ErrorMessage = "SubscriptionTypeId must be a long type"; }
        ...
        private ResultPackage<bool> Verify_PermissionName(object permission_name)
        {
            ...
            if (permission_name.GetType() == typeof(string))
            {
                ...
                if (string.IsNullOrEmpty(permission_name_str))
                    permission_name_result.ErrorMessage = "SubscriptionTypeName cannot be empty";
                else if (permission_name_str.Length > 100)
                    permission_name_result.ErrorMessage = "SubscriptionTypeName cannot exceed 100 characters";
                else if (permission_name_str.Any(c => c > 255))
                    permission_name_result.ErrorMessage = "SubscriptionTypeName cannot contain Unicode characters";
            }
            else { permission_name_result.ErrorMessage = "SubscriptionTypeName must be a string type"; }
