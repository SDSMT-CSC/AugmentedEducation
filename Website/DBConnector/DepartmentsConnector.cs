...
{
    class DepartmentsConnector : DBConnector
    {
        #region Members
        ...
        private string _Table_Name = "Departments";
        ...
        public ResultPackage<bool> Insert(long department_id, long organization_id, string department_name)
        {
            SqlCommand command = new SqlCommand(
                $"INSERT INTO {_Table_Name} (DepartmentId, DepartmentName, OrganizationId) " +
                "VALUES(@DepartmentId, @DepartmentName, @OrganizationId)");

            Add_Command_Parameter_With_Value(FieldNames.DepartmentId, department_id, ref command);
            Add_Command_Parameter_With_Value(FieldNames.DepartmentName, department_name, ref command);
        ...
        private ResultPackage<bool> Add_Command_Parameter_With_Value(FieldNames field_name, object value, ref SqlCommand command, string preferred_name = null)
        {
            ...
            if (value != null)
            {
                switch (field_name)
                {
                    case FieldNames.DepartmentName:
                        valid_value = Verify_DepartmentName(value);
                        if (string.IsNullOrEmpty(valid_value.ErrorMessage))
                        {
                            if (string.IsNullOrEmpty(preferred_name))
                                preferred_name = "@DepartmentName";
        ...
        private ResultPackage<bool> Verify_DepartmentName(object dept_name)
        {
            ResultPackage<bool> dept_name_result = new ResultPackage<bool>();
            string dept_name_str = string.Empty;

            if (dept_name.GetType() == typeof(string))
            {
                dept_name_str = (string)dept_name;

                if (string.IsNullOrEmpty(dept_name_str))
                    dept_name_result.ErrorMessage = "DepartmentName cannot be empty";
                else if (dept_name_str.Length > 100)
                    dept_name_result.ErrorMessage = "DepartmentName cannot exceed 100 characters";
                else if (dept_name_str.Any(c => c > 255))
                    dept_name_result.ErrorMessage = "DepartmentName cannot contain Unicode characters";

            }
            else { dept_name_result.ErrorMessage = "DepartmentName must be a string type"; }

            dept_name_result.ReturnValue = (string.IsNullOrEmpty(dept_name_result.ErrorMessage));

            return dept_name_result;
        }
