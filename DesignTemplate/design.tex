...
The web interface for users creates a data upstream exists between the web interface and cloud server. Users may upload, update, and delete remotely hosted files. 

Data downstream exists between the cloud server and AR hardware. Data downstream is initiated by users calling file download via QR codes or direct download link. 
...
When a file is accessed by an AR device, conversion software will intercede to convert the file to the appropriate AR compatible file type before delivery. This converted file will be stored on the cloud server for a limited time to streamline repeat or  multi-user access. 
