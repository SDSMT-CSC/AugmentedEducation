...
% must have a test identified, meaning you need to know how you plan to test it.
% If a requirement is not testable, then some justification needs to be made on 
...
which can be found in \autoref{sec:support}. Over the course of several 
meetings and conversations with the professors on the grant, we refined the 
requirements for our product.

An example user story from the grant, which is a close representation of the use 
case we are trying to support with our MVP, is as follows: 
"As a Calc III professor, I would like to provide visualizations of 3D objects
so that students can find the volume of an object by being able to view the
object from different angles, and being able to slice the object's volume"
...
% Integrating QR code reader
% View uploaded 3D models
...
% Break each point into subsection
\begin{itemize}
